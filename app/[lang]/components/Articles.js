...
const getArticles = async (locale) => {
  const api = `https://strapi.nikospap.blog/api/posts?filters[category][name][$eq]=articles&sort=publishedAt:desc&pagination[page]=1&pagination[pageSize]=4&populate=*&locale=${locale}`;
...
export default async function Articles({ locale }) {
  const { data } = await getArticles(locale);
  // console.log(data[0].attributes.localizations.data[0]);
  if (data.length === 0) {
    return (
      ...
      >
        <header>
          <Image
            ...
            alt="logo"
          />
          <h3 className="font-literata pt-3 text-slate-800 sm:font-bold tracking-wide	 hover:text-orange-700 text-lg sm:text-xl breal-all hyphens-auto line-clamp-3">
